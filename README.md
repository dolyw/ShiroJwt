...
```txt
登录认证通过后返回Token信息，同时在Redis中设置一条以帐号为Key的和Token过期时间一样的Value数据，现在认证时必须Token没失效以及
Redis中有存在当前帐号的Value数据才算认证通过，这样可以做到JWT的可控性，Redis中的Value数据就是在线用户，如果删除某个Value数据
，那这个Token之后也无法通过认证了，就相当于控制了用户的登录，可以剔除用户
```
